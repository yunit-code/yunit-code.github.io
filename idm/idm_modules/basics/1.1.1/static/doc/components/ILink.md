...
链接组件是一个展示带有链接的文本模块，通过给文本添加链接，实现文本向链接的转化，增加网页与网页间的互通性。链接组件的应用场景还是相当广泛的，在很多场景起到了网页与网页间桥梁的作用，比如在不便于放置图片、按钮的纯文本位置添加链接，仍然可以实现网页与网页间跳转的功能。
...
设置组件的链接类型，目前支持两种类型：默认、自定义。
如果需要更加高级、丰富的展示效果，需要以自定义实现
