...
(function() {
	...
	getFromDOM = function(key, json) {
		...
		el = document.querySelector("[data-idmpace-" + key + "]");
		if (!el) {
			return;
		}
		data = el.getAttribute("data-idmpace-" + key);
		if (!json) {
			return data;
		}
		try {
			...
			e = _error;
			return typeof console !== "undefined" && console !== null ? console.error("Error parsing inline idmpace options", e) : void 0;
		}
	};
	...
	idmPace = window.idmPace || {};

	window.idmPace = idmPace;

	extend(idmPace, Evented.prototype);

	options = idmPace.options = extend({}, defaultOptions, window.idmpaceOptions, getFromDOM());
	...
	Bar = (function() {
		...
		Bar.prototype.getElement = function() {
			...
			if (this.el == null) {
				...
				this.el.className = "idmpace idmpace-active";
				document.body.className = document.body.className.replace(/(idmpace-done )|/, 'idmpace-running ');
				var _custom_class_name = (options.className !== '') ? ' '+options.className : '';
				this.el.innerHTML = '<div class="idmpace-progress'+_custom_class_name+'">\n  <div class="idmpace-progress-inner"></div>\n</div>\n<div class="idmpace-activity"></div>';
		...
		Bar.prototype.finish = function() {
			var el;
			el = this.getElement();
			el.className = el.className.replace('idmpace-active', 'idmpace-inactive');
			return document.body.className = document.body.className.replace('idmpace-running ', 'idmpace-done ');
		};

		Bar.prototype.update = function(prog) {
			this.progress = prog;
			idmPace.trigger('progress', prog);
			return this.render();
		};
		...
		Bar.prototype.render = function() {
			...
			idmPace.trigger('change', this.progress);
	...
	getIntercept().on('request', function(_arg) {
		...
		if (!idmPace.running && (options.restartOnRequestAfter !== false || shouldTrack(type) === 'force')) {
			args = arguments;
	...
	idmPace.running = false;

	handlePushState = function() {
		if (options.restartOnPushState) {
			return idmPace.restart();
		}
	};
	...
	(init = function() {
		...
		idmPace.bar = bar = new Bar;
		scalers = [];
		return uniScaler = new Scaler;
	})();

	idmPace.stop = function() {
		idmPace.trigger('stop');
		idmPace.running = false;
		bar.update(100);
		bar.destroy();
	...
	idmPace.restart = function() {
		idmPace.trigger('restart');
		idmPace.stop();
		return idmPace.start();
	};

	idmPace.go = function() {
		var start;
		idmPace.running = true;
		bar.render();
		start = now();
		...
		return animation = runAnimation(function(frameTime, enqueueNextFrame) {
			...
			if (bar.done() || done || cancelAnimation) {
				bar.update(100);
				idmPace.trigger('done');
				return setTimeout(function() {
					bar.finish();
					idmPace.running = false;
					return idmPace.trigger('hide');
	...
	idmPace.start = function(_options) {
		...
		if (!document.querySelector('.idmpace')) {
			return setTimeout(idmPace.start, 50);
		} else {
			idmPace.trigger('start');
			return idmPace.go();
		}
	};
	...
	if (typeof define === 'function' && define.amd) {
		...
		module.exports = idmPace;
	} else {
		//设置颜色
		var openIdmCoreLoadStyle = true,idmCoreLoadColor = "#1890ff";
